...
import "./Menu.scss";
const Menu = ({menuHandler,showMenu}) => {
  const movies= ['Release Calender','Top 250 Movies','Most Popular Movies','Browser Movies by Genre','Top Box Office','Showtimes & Tickets', 'Movie News','India Movie Spotlight']
  const celebs=['Born Today','Most Popular Celebs','Celebrity News']
  const tvShows=[`What's on TV & Streaming`,'Top 250 TV Shows','Most Popular TV Shows','Browse TV Shows by Genre','TV News']
  const watch=['What to Watch','Latest Trailers','IMDb Originals','IMDb Picks','IMDb Spotlight','IMDb Podcasts']
  const award=['Oscars','Best of 2024','Holiday Picks','STARmeter Awards','Awards Central','Festive Central','All Events']
  const community=['Help Center','Contributor Zone','Polls']
  return (
    <>
    <div id="menuContainerBox">
      <div className={`menuContainer ${showMenu ? 'open' : 'close'}`}>
        ...
        <div className="menuBody">
          <div className="common column1">
            <div className="movies">
              <h2><i class="fa-solid fa-ticket"></i> Movies</h2>
              <ul>
                {
                  movies.map((list)=>(
                    <li><a href="/">{list}</a></li>
            ...
            <h2><i class="fa-solid fa-user-group"></i> Celebs</h2>
              <ul>
                {
                  celebs.map((list)=>(
                    <li><a href="/">{list}</a></li>
          ...
          <div className="common column2">
            ...
            <div className="watch">
            <h2><i class="fa-solid fa-circle-play"></i> Watch</h2>
              <ul>
                {
                  watch.map((list)=>(
                    <li><a href="/">{list}</a></li>
          ...
          <div className="common column3">
            <div className="award">
            <h2><i class="fa-solid fa-award"></i> Awards & Events</h2>
              <ul>
                {
                  award.map((list)=>(
                    <li><a href="/">{list}</a></li>
            ...
            <h2><i class="fa-solid fa-globe"></i> Community</h2>
              <ul>
                {
                  community.map((list)=>(
                    <li><a href="/">{list}</a></li>
