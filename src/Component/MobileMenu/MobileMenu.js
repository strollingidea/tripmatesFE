...
const MobileMenu = ({ mobMenuHandler }) => {
  ...
  const accordionData = [
    ...
    },
    {
      ...
      title: "Celebs",
      content: ["Born Today", "Most Popular Celebs", "Celebrity News"],
    },
    {
      icon: <i class="fa-solid fa-globe"></i>,
      title: "Community",
      content: ["Help Center", "Contributor Zone", "Polls"],
    },
  ];
  ...
  // Get the max height of content based on whether it's active or not
  const getContentMaxHeight = (index) => {
    const content = contentRefs.current[index];
    return content && content.scrollHeight ? content.scrollHeight : 0;
  };
  ...
  return (
    <div id="accordionsection">
      <div className="closingBtn">
        ...
        <label>
          <i class="fa-solid fa-xmark" onClick={() => mobMenuHandler()}></i>
        </label>
      </div>
          ...
          <div key={index} className="accordion-item">
            ...
            >
              <h3 className="accordion-heading">
                <span className="opct">{item.icon}</span>
                <span className="headingTitle">{item.title}</span>
              </h3>
              <span
                className={`opct accordion-btn transition-transform duration-300 ${
            ...
            <div
              ...
              style={{
                maxHeight:
                  activeIndex === index ? getContentMaxHeight(index) : 0,
                opacity: activeIndex === index ? 1 : 0,
                visibility: activeIndex === index ? "visible" : "hidden", // Hide completely when closed
                transition:
                  "max-height 300ms cubic-bezier(0.4, 0, 0.2, 1), opacity 300ms cubic-bezier(0.4, 0, 0.2, 1), visibility 300ms ease-out",
                overflow: "hidden", // Ensure content doesn't overflow
              }}
            >
              <ul className="accordion-list">
                {item.content.map((listItem, contentIndex) => (
                  <li key={contentIndex} className="accordion-list-item">
                    <a href="">{listItem}</a>
                  </li>
                ))}
              </ul>
